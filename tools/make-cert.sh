...
#
# You may not need this at all. If the machine is on a tailnet, `tailscale
# serve` gives you a real, publicly-trusted certificate with nothing to install
# on the phone — see README.md. This script is for the case where it is not.
#
...
# not trust — "An SSL certificate error occurred when fetching the script" — so
# the offline install silently never happens. Installing ssl/ca.pem on the phone
# once fixes that, and a certificate can only be installed as a trust anchor if
# it is marked as a CA, which a plain self-signed leaf is not.
#
...
# The tailnet name, if there is one. A certificate that does not name it is
# refused by the browser the moment you open the machine's MagicDNS address,
# which is the address most likely to be typed on the phone.
ts_name=""
...
if [ -n "${TAILSCALE_HOSTNAME:-}" ]; then ts_name="$TAILSCALE_HOSTNAME"; fi
if [ -n "$ts_name" ]; then
  alt="$alt,DNS:$ts_name"
fi
...
Restart the server and it serves https on port ${HTTPS_PORT:-8453} alongside http.
